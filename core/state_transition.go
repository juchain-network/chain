...
func (st *StateTransition) preCheck() error {
	...
	if st.evm.ChainConfig().IsBlacklistV1(st.evm.Context.BlockNumber) {
		...
		if params.InBlacklistV1(msg.From, *toAddress) {
			return fmt.Errorf("%w: from %v, to %v", ErrBlacklistAddr, msg.From, *toAddress)
		}
	}
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	} else if st.evm.ChainConfig().Congress != nil {
		fee := new(uint256.Int).SetUint64(st.gasUsed())
		fee.Mul(fee, effectiveTipU256)
		st.state.AddBalance(params.FeeRecorder, fee)
	} else {
		fee := new(uint256.Int).SetUint64(st.gasUsed())
		fee.Mul(fee, effectiveTipU256)
