...
func ValidateTransaction(tx *types.Transaction, head *types.Header, signer types.Signer, opts *ValidationOptions) error {
	...
	// Changed to use fork rules for judgment
	if rules.IsBlackList {
		toAddress := new(common.Address)
		if tx.To() != nil {
			toAddress = tx.To()
		}
		if params.InBlacklistV1(from, *toAddress) {
			return core.ErrBlacklistAddr
		}
	}
	...
	if tx.GasFeeCapIntCmp(big.NewInt(0).Add(opts.MinTip, minBaseFee)) < 0 {
		return fmt.Errorf("%w: gas fee cap %v, gas tip cap %v", ErrUnderpriced, tx.GasFeeCap(), tx.GasTipCap())
	}
...
func ValidateTransactionWithState(tx *types.Transaction, signer types.Signer, opts *ValidationOptionsWithState) error {
	if tx.Type() == types.BlobTxType {
		return errors.New("blob transactions are disabled on this chain")
	}
	...
	// Check if transaction is in blacklist
	if core.IsAddressBlacklisted(from, tx.To()) {
		return core.ErrBlacklistAddr
	}
