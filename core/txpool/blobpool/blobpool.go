...
func (p *BlobPool) Reset(oldHead, newHead *types.Header) {
	...
	if p.chain.Config().IsCancun(p.head.Number, p.head.Time) {
		final := p.chain.CurrentFinalBlock()
		// The first block after fork may be empty, avoid incorrect calls
		if final != nil {
			p.limbo.finalize(final)
		} else {
			log.Warn("Skipping blob limbo finalize due to missing parent", "number", p.head.Number)
		}
	}
