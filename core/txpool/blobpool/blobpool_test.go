...
func init() {
	...
	*testChainConfig = *params.MainnetChainConfig
	...
	// Enable all required forks for blob transactions
	testChainConfig.HomesteadBlock = big.NewInt(0)
	testChainConfig.EIP150Block = big.NewInt(0)
	testChainConfig.EIP155Block = big.NewInt(0)
	testChainConfig.EIP158Block = big.NewInt(0)
	testChainConfig.ByzantiumBlock = big.NewInt(0)
	testChainConfig.ConstantinopleBlock = big.NewInt(0)
	testChainConfig.PetersburgBlock = big.NewInt(0)
	testChainConfig.IstanbulBlock = big.NewInt(0)
	testChainConfig.BerlinBlock = big.NewInt(0)
	testChainConfig.LondonBlock = big.NewInt(0)

	testChainConfig.CancunTime = new(uint64)
	*testChainConfig.CancunTime = uint64(1000) // Use a small fixed timestamp
}
...
func (bc *testBlockChain) CurrentBlock() *types.Header {
	...
	return &types.Header{
		...
		Difficulty:    big.NewInt(0), // Post-merge difficulty is always 0
	}
}
