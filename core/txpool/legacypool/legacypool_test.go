...
func init() {
	...
	// Set PriceLimit to 1 (minimum allowed by sanitize function) for Congress network compatibility
...
func (bc *testBlockChain) CurrentBlock() *types.Header {
	return &types.Header{
		...
		BaseFee:    new(big.Int), // Always set BaseFee to 0 for tests
	}
}
...
func testQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	if err := pool.addLocal(pricedTransaction(1, 100000, big.NewInt(10), local)); err != nil {
		t.Fatalf("failed to add local transaction: %v", err)
	}
	if err := pool.addRemote(pricedTransaction(1, 100000, big.NewInt(10), remote)); err != nil {
	...
	if err := pool.addLocal(pricedTransaction(4, 100000, big.NewInt(10), local)); err != nil {
		t.Fatalf("failed to add remote transaction: %v", err)
	}
	if err := pool.addRemoteSync(pricedTransaction(4, 100000, big.NewInt(10), remote)); err != nil {
		t.Fatalf("failed to add remote transaction: %v", err)
	}
	time.Sleep(5 * evictionInterval) // A half lifetime pass

	// Queue executable transactions, the life cycle should be restarted.
	if err := pool.addLocal(pricedTransaction(2, 100000, big.NewInt(10), local)); err != nil {
		t.Fatalf("failed to add remote transaction: %v", err)
	}
	if err := pool.addRemoteSync(pricedTransaction(2, 100000, big.NewInt(10), remote)); err != nil {
		t.Fatalf("failed to add remote transaction: %v", err)
	}
