...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *triedb.Database, genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig, common.Hash, error) {
	...
	// apply the overrides.
	// Merge the stored genesis config from block 0 in database with override config passed in
	...
	// If the merged config differs from the genesis config, update the local database
...
func (g *Genesis) ToBlock() *types.Block {
	...
	if conf := g.Config; conf != nil {
		...
		// Genesis timestamp is 0, so Shanghai and Cancun forks will never be satisfied, the flow below won't execute
...
// Continue using old format reading logic here, new format requires changes to genesis.json file structure
func decodePrealloc(data string) types.GenesisAlloc {
	...
	log.Info("Decode prealloc data", "data", data)
