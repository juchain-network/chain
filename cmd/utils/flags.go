...
var (
	...
	RPCGlobalMaxBlockSpanFlag = &cli.Uint64Flag{
		Name:     "rpc.blockspan",
		Usage:    "Sets the maximum block span range allowed for RPC interface queries",
		Value:    ethconfig.Defaults.RPCMaxBlockSpan,
		Category: flags.APICategory,
	}
	...
	MaxFastPeersFlag = &cli.IntFlag{
		Name:     "maxfastpeers",
		Usage:    "Maximum number of direct broadcast peers (defaults used if set to 0)",
		Value:    node.DefaultConfig.P2P.MaxFastPeers,
		Category: flags.NetworkingCategory,
	}
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	if ctx.IsSet(MaxFastPeersFlag.Name) {
		cfg.MaxFastPeers = ctx.Int(MaxFastPeersFlag.Name)
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	if ctx.IsSet(RPCGlobalMaxBlockSpanFlag.Name) {
		cfg.RPCMaxBlockSpan = ctx.Uint64(RPCGlobalMaxBlockSpanFlag.Name)
	}
	if ctx.IsSet(NoDiscoverFlag.Name) {
...
func RegisterFilterAPI(stack *node.Node, backend ethapi.Backend, ethcfg *ethconfig.Config) *filters.FilterSystem {
	...
	stack.RegisterAPIs([]rpc.API{{
		Namespace: "eth",
		Service:   filters.NewFilterAPI(filterSystem, false, ethcfg.RPCMaxBlockSpan),
	}})
	return filterSystem
}
...
func MakeChain(ctx *cli.Context, stack *node.Node, readonly bool) (*core.BlockChain, ethdb.Database) {
	...
	// Bugfix: Set congress consensus state function to avoid errors during block data import
	if congressEngine, ok := engine.(*congress.Congress); ok {
		congressEngine.SetStateFn(chain.StateAt)
	}
