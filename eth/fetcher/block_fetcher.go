	...
	for {
		...
		for !f.queue.Empty() {
			...
			if f.light {
				...
				log.Debug("block_fetcher call import blocks", "number", op.block.Number(), "hash", op.block.Hash(), "miner", op.block.Coinbase(), "withdrawHash", op.block.Header().WithdrawalsHash, "withdrawals", op.block.WithdrawalStr())
		...
		case filter := <-f.headerFilter:
			...
			for _, header := range task.headers {
				...
				log.Debug("block_fetcher filter header", "number", header.Number, "hash", header.Hash(), "miner", header.Coinbase, "withdrawHash", header.WithdrawalsHash)
				...
				if announce := f.fetching[hash]; announce != nil && announce.origin == task.peer && f.fetched[hash] == nil && f.completing[hash] == nil && f.queued[hash] == nil {
					...
					if f.getBlock(hash) == nil {
						...
						if header.TxHash == types.EmptyTxsHash && header.UncleHash == types.EmptyUncleHash {
							...
							// If it's an empty block without transactions, only header info is available here
							// But subsequent processing needs WithdrawalsHash, so we need to set an empty value here
							if header.WithdrawalsHash != nil && *header.WithdrawalsHash == types.EmptyWithdrawalsHash {
								//log.Warn("header.WithdrawalsHash is EmptyWithdrawalsHash and block is empty, set block Withdrawal to empty!")
								block.SetWithdrawals([]*types.Withdrawal{})
							}
				...
				for i := 0; i < len(task.transactions) && i < len(task.uncles); i++ {
					...
					for hash, announce := range f.completing {
						...
						if f.getBlock(hash) == nil {
							...
							block.ReceivedAt = task.time
							// Blocks constructed here don't contain withdrawals (empty), need to check if header has Hash set and fill it
							if block.Header().WithdrawalsHash != nil && *block.Header().WithdrawalsHash == types.EmptyWithdrawalsHash && block.Withdrawals() == nil {
								block.SetWithdrawals([]*types.Withdrawal{})
							}
							blocks = append(blocks, block)
							log.Debug("block_fetcher mark the body matched and reassemble", "number", block.Number(), "hash", block.Hash(), "miner", block.Coinbase(), "withdrawHash", block.Header().WithdrawalsHash, "withdrawals", block.WithdrawalStr())
						} else {
							f.forgetHash(hash)
						}
					}
...
func (f *BlockFetcher) importBlocks(peer string, block *types.Block) {
	...
	go func() {
		...
		// Run the actual import and log any issues
		log.Debug("block_fetcher run the actual import", "number", block.Number(), "hash", block.Hash(), "miner", block.Coinbase(), "withdrawHash", block.Header().WithdrawalsHash, "withdrawals", block.WithdrawalStr())
