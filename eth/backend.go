...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	// Load ChainConfig here, attach genesis config
	...
	if congressEngine, ok := eth.engine.(*congress.Congress); ok {
		...
		congressEngine.SetChainConfig(eth.blockchain.Config())
	}
	...
	if err := core.InitBlacklist(eth.blockchain); err != nil {
...
func (s *Ethereum) Start() error {
	...
	maxFastPeers := s.p2pServer.MaxFastPeers
	if s.config.LightServ > 0 {
		if s.config.LightPeers >= s.p2pServer.MaxPeers {
			return fmt.Errorf("invalid peer config: light peer count (%d) >= total peer count (%d)", s.config.LightPeers, s.p2pServer.MaxPeers)
		}
		maxPeers -= s.config.LightPeers
		maxFastPeers -= s.config.LightPeers
	}
	if maxFastPeers < 0 {
		maxFastPeers = 0
	}
	// Start the networking layer and the light server if requested
	s.handler.Start(maxPeers, maxFastPeers)
	return nil
}
