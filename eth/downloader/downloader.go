...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	for i, result := range results {
		...
		log.Debug("block_fetcher downloader import block results", "number", blocks[i].Number(), "hash", blocks[i].Hash(), "miner", blocks[i].Coinbase(), "withdrawHash", blocks[i].Header().WithdrawalsHash, "withdrawals", blocks[i].WithdrawalStr())
	}
