...
func (h *handler) Start(maxPeers, maxFastPeers int) {
	h.maxPeers = maxPeers
	h.maxFastPeers = maxFastPeers
...
func (h *handler) getMaxFastPeers(peers []*ethPeer) int {
	peerCount := len(peers)
	if h.maxFastPeers <= 0 {
		limit := int(math.Sqrt(float64(peerCount)))
		if limit > peerCount {
			limit = peerCount
		}
		return limit
	}
	if h.maxFastPeers > peerCount {
		return peerCount
	}
	return h.maxFastPeers
}
...
func (h *handler) BroadcastBlock(block *types.Block, propagate bool) {
	...
	if propagate {
		...
		transfer := peers[:h.getMaxFastPeers(peers)]
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	...
	for _, tx := range txs {
		...
		case tx.Size() > txMaxBroadcastSize:
			largeTxs++
		default:
			numDirect = h.getMaxFastPeers(peers)
		}
