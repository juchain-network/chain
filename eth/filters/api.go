...
type FilterAPI struct {
	...
	rpcBlockRange uint64 // RPC maximum block query range
}

// NewFilterAPI returns a new FilterAPI instance.
func NewFilterAPI(system *FilterSystem, lightMode bool, rpcBlockRange uint64) *FilterAPI {
	api := &FilterAPI{
		sys:           system,
		events:        NewEventSystem(system, lightMode),
		filters:       make(map[rpc.ID]*filter),
		timeout:       system.cfg.Timeout,
		rpcBlockRange: rpcBlockRange,
	}
...
func (api *FilterAPI) GetLogs(ctx context.Context, crit FilterCriteria) ([]*types.Log, error) {
	...
	if crit.BlockHash != nil {
		...
		err := api.checkBlockRange(ctx, begin, end)
		if err != nil {
			return nil, err
		}
...
func (api *FilterAPI) checkBlockRange(ctx context.Context, begin, end int64) error {
	maxSpan := api.rpcBlockRange
	if maxSpan > 1 {
		if end == rpc.LatestBlockNumber.Int64() || begin == rpc.LatestBlockNumber.Int64() {
			header, _ := api.sys.backend.HeaderByNumber(ctx, rpc.LatestBlockNumber)
			if header == nil {
				return fmt.Errorf("failed to get latest block number")
			}
		...
		blockRange := end - begin
		if blockRange > 1 && blockRange > int64(maxSpan) {
			log.Warn("block range too large", "range", blockRange)
			return fmt.Errorf("block range too large, max is %d", maxSpan)
		}
	}
	return nil
}
